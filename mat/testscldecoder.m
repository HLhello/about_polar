...
for ii = 0:log2(N)-1
    step = N/2^ii;
    LLR_temp = llr_mat(1,step_all+1:step_all+step);
    step_all = step_all + step;
    for jj = 1:N/2^(ii+1)
        llr_mat(step_all+jj) = sign( LLR_temp(2*jj-1) .* LLR_temp(2*jj) ) .* min( abs(LLR_temp(2*jj-1)),abs(LLR_temp(2*jj)) );
    end
end
...
for i = 1:N
    if info_position(i)==0
        if i==1
            ...
            u = 0;
            llr_mat = mat_init;
            LLR = likelihood_rate(N,i,uu,llr_mat);
            PM_former = 0;
            for j = 1:list_len
        ...
        path_No = path_No+1;
        if path_No<=allpath                   %此时要保留所有路径 并算概率
             u_mat(:,i) = reserve_all(:,path_No);
             for j = 1:list_len
                 uu = u_mat(j,1:i-1);
                 u = u_mat(j,i);
                 llr_mat = mat_init;
